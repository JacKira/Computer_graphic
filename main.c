...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        if(loadDiffuseMap(model, argv[3]) == 0){
...
void render(tgaImage *image, Model *model)
{
    ...
    for (face = 0; face < model->nface; ++face) {
        ...
        for (vert = 0; vert < 3; ++vert) {
            ...
            screen_coords[vert][0] = ((*p[vert])[0] / scale + 1.0) * w / 2;
            screen_coords[vert][1] = ((*p[vert])[1] / scale + 1.0) * h / 2;
            screen_coords[vert][2] = (*p[vert])[2] * 1e6;
